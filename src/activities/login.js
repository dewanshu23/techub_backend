...
const login = async (req, res) => {
    ...
    try {
        ...
        if (!isValidPassword) {
            await logEntry({ user_id: existingUser.id, activity: 'Login failed Password does not match for email ' + email + ' and userid ' + existingUser.id });
        ...
        await logEntry({ user_id: existingUser.id, activity: 'Login successful for email ' + email + ' and userid ' + existingUser.id });
        res.status(200).json({ message: 'Login successful', token, user_data: existingUser });
    } catch (err) {
        console.error(err);
        await logEntry({ user_id: existingUser!=undefined?existingUser.id:0, activity: 'Login failed Internal server error' });
